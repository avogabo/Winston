...
import { AlertTriangle, CheckCircle2, Film, FolderTree, LoaderCircle, Search, Settings2, ShieldCheck, Sparkles, Tv, Wand2 } from 'lucide-react'
import './app.css'
...
const defaultSettings: Settings = {
  ...
  movies_template: '',
  series_template: '',
  filebot_movie_format: '',
  filebot_series_format: '',
...
export default function App() {
  ...
  async function saveSettings() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'No pude guardar ajustes')
    } finally {
    ...
    <div className="app-shell">
      <section className="hero">
        ...
        <div className="hero-copy">
          ...
          <p>Winston autoimporta cuando está claro, frena cuando duda y te deja corregir por TMDB ID, path final y ajustes persistentes.</p>
      ...
      <section className="tabs-row">
        <button className={`tab-btn ${tab === 'review' ? 'active' : ''}`} onClick={() => setTab('review')}>Revisión</button>
        <button className={`tab-btn ${tab === 'settings' ? 'active' : ''}`} onClick={() => setTab('settings')}><Settings2 size={16} /> Ajustes</button>
      </section>
      ...
      {tab === 'settings' && (
        <section className="settings-layout glass">
          <div className="panel-head">
            <div>
              <h2>Ajustes</h2>
              <p>Configuración persistente para AltMount, Plex, FileBot, naming y política de import.</p>
            </div>
            <button className="primary-btn" onClick={() => void saveSettings()} disabled={savingSettings}>
          ...
          <div className="settings-grid">
            <SettingsCard title="AltMount">
              <Field label="Base URL" value={settings.altmount_base_url} onChange={(value) => setSettings({ ...settings, altmount_base_url: value })} placeholder="http://altmount:8080" />
              <Field label="API Key" value={settings.altmount_api_key} onChange={(value) => setSettings({ ...settings, altmount_api_key: value })} placeholder="token" />
              <Field label="Modo por defecto" value={settings.default_mode} onChange={(value) => setSettings({ ...settings, default_mode: value })} placeholder="preserve | template | filebot" />
              <Field label="Sleep entre imports" value={settings.sleep_between_imports} onChange={(value) => setSettings({ ...settings, sleep_between_imports: value })} placeholder="3s" />
            </SettingsCard>

            <SettingsCard title="Plex">
              <Field label="Base URL" value={settings.plex_base_url} onChange={(value) => setSettings({ ...settings, plex_base_url: value })} placeholder="http://plex:32400" />
              <Field label="Token" value={settings.plex_token} onChange={(value) => setSettings({ ...settings, plex_token: value })} placeholder="plex-token" />
              <Field label="Path from" value={settings.plex_path_from} onChange={(value) => setSettings({ ...settings, plex_path_from: value })} placeholder="/home" />
              <Field label="Path to" value={settings.plex_path_to} onChange={(value) => setSettings({ ...settings, plex_path_to: value })} placeholder="/media/biblioteca" />
            </SettingsCard>

            <SettingsCard title="Naming y FileBot">
              <Field label="Movies template" value={settings.movies_template} onChange={(value) => setSettings({ ...settings, movies_template: value })} placeholder="Peliculas/{quality}/{alpha}/{title} ({year})" />
              <Field label="Series template" value={settings.series_template} onChange={(value) => setSettings({ ...settings, series_template: value })} placeholder="Series/{alpha}/{series}/Temporada {season}/{series} - {episode}" />
              <Field label="FileBot movie format" value={settings.filebot_movie_format} onChange={(value) => setSettings({ ...settings, filebot_movie_format: value })} placeholder="Peliculas/{plex}" />
              <Field label="FileBot series format" value={settings.filebot_series_format} onChange={(value) => setSettings({ ...settings, filebot_series_format: value })} placeholder="Series/{plex}" />
              <Field label="FileBot DB" value={settings.filebot_db} onChange={(value) => setSettings({ ...settings, filebot_db: value })} placeholder="TheMovieDB" />
            </SettingsCard>

            <SettingsCard title="Política de matching">
              <label className="check-row">
                <input type="checkbox" checked={settings.auto_import_medium} onChange={(e) => setSettings({ ...settings, auto_import_medium: e.target.checked })} />
                <span>Autoimportar `medium` si no hay conflicto</span>
              </label>
            </SettingsCard>
          </div>
        </section>
