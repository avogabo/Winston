...
const defaultSettings: Settings = {
  ...
  default_mode: 'filebot', sleep_between_imports: '3s', movies_template: '', series_template: '', filebot_movie_format: '', filebot_series_format: '', filebot_db: 'TheMovieDB', filebot_binary: '/usr/local/bin/filebot', filebot_home: '/config/filebot', auto_import_medium: true,
}
...
export default function App() {
  ...
  async function rescanItems() {
    ...
    try {
      const res = await fetch('/api/review/rescan', { method: 'POST' })
      if (!res.ok) throw new Error(await res.text())
      const data: ReviewListResponse = await res.json()
      setItems(data.items)
      if (!selectedSource && data.items.length > 0) setSelectedSource(data.items[0].source_nzb_path)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'No pude buscar nuevos NZB')
    } finally {
      setRescanning(false)
    }
  }
